...
The repository now uses a **single launcher** for both experiences. By default the CRM opens, but you can opt into the sales workflow by setting `PS_APP=sales` (or `PS_APP_SCRIPT=sales_app.py` when using the desktop launcher) before starting the application. Everything shares one `requirements.txt`, `Procfile`, and Render/Railway config—there is no longer a second app to deploy separately. Render deployments start via `render_bootstrap.py`, which enforces headless mode/disabled analytics and skips installing signal handlers when the platform runs Streamlit in a worker thread (a common cause of `ValueError: signal only works in main thread` on Python 3.13).
